...
function fillTable(){
    ...
    //Début Traitements
        ...
        for(i=0; i<dataLength; i++){
            var t1 = new Date(sessionStorage.getItem(`date${i}`));
            var t2 = t1.toLocaleDateString();
            document.getElementById("stateOneTableBody").innerHTML += `<tr><td>${i+1}</td><td>${sessionStorage.getItem(`name${i}`)}</td><td>${sessionStorage.getItem(`firstName${i}`)}</td><td>${sessionStorage.getItem(`center${i}`)}</td><td>${t2}</td></tr>`
        };
